  ...
  return (
    <div className={`bg-white dark:bg-neutral`}>
      <div className="font-[sans-serif] p-4 mx-auto lg:max-w-5xl md:max-w-3xl max-w-lg">
        <h2 className="text-4xl font-extrabold text-[#121212] mb-12  dark:text-[#3A80F6]">
          Latest Product
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {LatestProduct.data.map((item, index) => (
            <div
              key={index}
              className="bg-[#f0f0f0] dark:bg-[#2c2c2c] rounded-md overflow-hidden cursor-pointer hover:shadow-lg transition-all relative"
            >
              <div className="w-full h-[250px] overflow-hidden mx-auto aspect-w-16 aspect-h-8 p-2">
                <img
                  src={`http://localhost:4000/images/products/${item.img}`}
                  alt="laptop1"
              ...
              <div className="p-6">
                ...
                <div>
                  <h3 className="text-base text-[#121212] dark:text-[#3A80F6]">
                    {item.title}
                  </h3>
                  <h4 className="text-lg text-[#121212] dark:text-[#3A80F6] font-bold my-6">
                    ${item.sale_price}{" "}
                    <strike className="text-[#121212] dark:text-[#3A80F6] ml-2 font-medium">
                      ${item.old_price}
                    </strike>
                  </h4>
                </div>
