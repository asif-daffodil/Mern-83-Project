...
const LatestProducts = () => {
    ...
    return (
        <div className="font-[sans-serif] p-4 mx-auto lg:max-w-5xl md:max-w-3xl max-w-lg">
            <h2 className="text-4xl font-extrabold text-white mb-12  dark:text-[#3A80F6]">Latest Product</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
             {LatestProduct.data.map((item, index) => (
                <div key={index} className="bg-[#0D1117]  overflow-hidden cursor-pointer hover:shadow-lg transition-all relative">
                    ...
                    <div className="p-6">
                        ...
                        <div>
                            <h3 className="text-base text-[#FFF] dark:text-[#3A80F6]">{item.title}</h3>
                            <h4 className="text-lg text-[#FFF] dark:text-[#3A80F6] font-bold my-6">${item.sale_price} <strike
                                className="text-[#FFF] dark:text-[#3A80F6] ml-2 font-medium">${item.old_price}</strike>
                            </h4>
                        </div>
